...
{
	global $htUseHistory, $htScriptPath;
...
function renderSwitchTabLink(&$parser, $tabName, $linkText, $anotherTarget)
{
	...
	if ($anotherTarget != '')
	{
		...
		$targetKey = $targetTitle->getDBkey();

		$targetcode = ", '$targetKey'";
	}

	$output = '<a href="#tab='.$tabKey.'" onclick="return HeaderTabs.switchTab(\''.$tabKey.'\''.$targetcode.')">'.$linkText.'</a>';
